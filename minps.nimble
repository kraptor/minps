...
task build_debug, "Build debug version":
    exec "nimble -d:debug --gc:orc --debugger:native --debuginfo --linedir:on --threads:on -d:MINPS_DEBUG -d:Version:" & version & " build"
    appendBinaries "_debug"

task build_release, "Build release version":
    exec "nimble -d:danger --gc:orc --opt:speed --passC:-O3 --threads:on -d:MINPS_RELEASE -d:Version:" & version & " build"
...
task build_release_stacktrace, "Build release version (with stacktraces)":
    exec "nimble -d:danger --gc:orc --stackTrace:on --opt:speed --passC:-O3 --threads:on -d:MINPS_RELEASE -d:Version:" & version & " build"
    appendBinaries "_release_stacktrace"

task build_profiler, "Build with profiler":
    exec "nimble -d:danger --gc:orc --profiler:on --stackTrace:on -d:MINPS_PROFILER --threads:on -d:Version:" & version & " build"
    appendBinaries "_profiler"

task build_profiler_memory, "Build with memory profiler":
    exec "nimble -d:danger --gc:orc --profiler:off -d:memProfiler --stackTrace:on --threads:on -d:MINPS_PROFILER_MEMORY -d:Version:" & version & " build"
